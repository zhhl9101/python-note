...
**如果去掉装饰器`@classmethod`，这段代码还能运行吗？**像这样：
...
实际场景中我们一定会处理权限问题和日志问题，在这类问题中会涉及到每个方法，需要对每个方法都进行相应的处理，这个时候装饰器是最优的选择。
...
现在要做权限认证，只允许1-10岁的小朋友观看，或者只允许管理员观看等。

这里我们可以直接修改`play()`方法，但是如果我有若干个方法都需要做权限校验，难道要一个个的改吗？
